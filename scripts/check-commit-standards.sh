...
artifact_pattern='(IBX|RSH|DEC|LOG|UPS)-[0-9]{8}-[0-9]{3}'
...
fail() {
  ...
  echo "  project: $expected_project" >&2
...
echo "$artifacts" | grep -Eq "^$artifact_pattern(, $artifact_pattern)*$" ||
  fail "artifacts trailer must be a comma-separated list like DEC-YYYYMMDD-NNN"
