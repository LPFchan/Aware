...
- Current focus: use repo-root entrypoints, local writing guides, and enforced commit provenance to keep future repo changes aligned with repo-template while maintaining the shipping macOS app and release automation
...
- Current work: repo-root `AGENTS.md` and `CLAUDE.md` now enforce the writing contract, and touched guides have been normalized toward repo-template
...
- Date: `2026-04-09`
  - Change: added the repo operating model overlay at the repo root plus bootstrap decision and worklog records
...
- Blocker or risk: commit-provenance checks currently run remotely on pull requests and default-branch pushes, not every possible branch or tag push
  ...
  - Owner: operator
  - Mitigation: keep local hook installation easy, keep PR checks required, and revisit CI scope later if the workflow needs broader remote coverage
